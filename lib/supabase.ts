...
// service_role_key deve estar APENAS em .env local (VITE_SUPABASE_SERVICE_ROLE_KEY).
const supabaseServiceRoleKey: string = env.VITE_SUPABASE_SERVICE_ROLE_KEY || '';
export const supabaseAdmin = supabaseServiceRoleKey
  ? createClient(supabaseUrl, supabaseServiceRoleKey, {
      auth: { autoRefreshToken: false, persistSession: false }
    })
  : supabase; // Fallback seguro: usa cliente anon (operacoes admin falharao com permissao negada)
