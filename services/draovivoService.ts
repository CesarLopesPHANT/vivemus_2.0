...
const getConfig = async (): Promise<DrAoVivoConfig> => {
  // SEGURANCA: A API key DAV NUNCA deve ser exposta no frontend.
  // Em producao, todas as chamadas a API DAV devem ser feitas via Edge Functions.
  // O fluxo principal (PSO) ja usa a Edge Function pso-proxy.
  const isBrowser = typeof window !== 'undefined';

  // Em ambiente browser, retorna vazio para forcar modo simulado
  if (isBrowser) {
    return { endpoint: BASE_URL, apiKey: '' };
  }

  // Server-side: busca da variavel de ambiente
